...
{
    private Tower thisTower;        //같은 오브젝느 안에 있는 컨포넌트 타워에 접근

    public GameObject projectile;   //발사체 프리팹
    public Transform firePoint;     //발사 위치 Transform 값
    ...
    void Update()
    {
        ...
        {
            launcherModel.rotation = Quaternion.Slerp(launcherModel.rotation, Quaternion.LookRotation(target.position - transform.position),
            5f * Time.deltaTime);
